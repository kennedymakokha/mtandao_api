...
import { TownsArray } from "./townsArray";
import Towns from "../models/towns.model";
...
export const Create = async (req: Request | any, res: Response) => {
    try {
        for (let element of TownsArray) {
            const existingTown = await Towns.findOne({ name: element.name });

            if (!existingTown) {
                const townData = {
                    town_name: element.name,
                    location: {
                        lng: element.lng,
                        lat: element.lat
                    },
                ...
                await Towns.create(townData);
            }
        }
        res.status(201).json("Towns created successfully");
        return;
    } catch (error) {
        console.log(error)
...
export const Get = async (req: Request | any, res: Response | any) => {
    try {
        ...
        const towns: any = await Towns.find({ deletedAt: null }).sort({ town_name: 1 }).skip((page - 1) * limit)
            .limit(parseInt(limit))
            .sort({ createdAt: -1 })
        const total = await Towns.countDocuments();
        res.status(201).json({
            towns, page: parseInt(page),
