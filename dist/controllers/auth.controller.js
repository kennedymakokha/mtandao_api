...
const generate_activation_util_1 = require("../utils/generate_activation.util");
const admin_model_1 = require("../models/admin.model");
// User Registration
const register = async (req, res) => {
    try {
        const { username, password, phone_number } = req.body;
        let phone = await (0, simplefunctions_util_1.Format_phone_number)(phone_number); //format the phone number
...
const updatePassword = async (req, res) => {
    try {
        ...
        let phone = await (0, simplefunctions_util_1.Format_phone_number)(phone_number);
        const user = await user_model_1.User.findOne({ phone_number: phone }); // Find the user by ID
        if (!user) {
...
const activateuser = async (req, res) => {
    try {
        ...
        let phone = await (0, simplefunctions_util_1.Format_phone_number)(phone_number);
        const user = await user_model_1.User.findOne({ phone_number: phone });
        if (!user) {
            res.status(400).json("user not found");
            return;
        }
...
const login = async (req, res) => {
    try {
        ...
        if (!userExists || !(await bcryptjs_1.default.compare(password, userExists.password))) {
            ...
            const { accessToken, refreshToken } = (0, generatetoken_util_1.default)(userExists);
...
const registerAdmin = async (req, res) => {
    try {
        const { username, password, phone_number } = req.body;
        let newID = ` ${Date()}-${(0, generate_activation_util_1.MakeActivationCode)(4)}`;
        req.body.username = "Admin";
        let phone = await (0, simplefunctions_util_1.Format_phone_number)(phone_number); //format the phone number
        const userExists = await admin_model_1.Admin.findOne({
            $or: [
                { phone_number: phone }
            ],
        });
        ...
        req.body.phone_number = phone;
        const user = new admin_model_1.Admin(req.body);
...
const admin_login = async (req, res) => {
    try {
        ...
        const { phone_number, password } = req.body;
        let phone = await (0, simplefunctions_util_1.Format_phone_number)(phone_number); //format the phone number
        const userExists = await admin_model_1.Admin.findOne({ phone_number: phone });
        if (!userExists || !(await bcryptjs_1.default.compare(password, userExists.password))) {
            res.status(401).json("Invalid credentials");
            return;
        }
...
const get_Users = async (req, res) => {
    try {
        ...
        let users = await user_model_1.User.find().select(`-password ${sendId ? "" : "-_id"} -updatedAt -__v`)
            .skip((page - 1) * limit)
...
const get_Admins = async (req, res) => {
    try {
        ...
        let users = await admin_model_1.Admin.find().select('-password -_id -updatedAt -__v -activated')
