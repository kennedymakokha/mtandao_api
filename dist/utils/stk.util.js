...
const Mpesa_stk = async (No, amount, user) => {
    ...
    const consumer_secret = process.env.MPESA_CONSUMER_SECRETE;
    const passkey = process.env.MPESA_CONSUMER_PASSKEY;
    const short_code = parseInt(process.env.MPESA_SHORT_CODE, 10);
    ...
    const fetch_response = await (0, node_fetch_1.default)(`${process.env.MPESA_BASE_URL}/mpesa/stkpush/v1/processrequest`, {
        method: "POST",
        headers,
        body: JSON.stringify({
    ...
    await new mpesa_logs_model_1.default({
