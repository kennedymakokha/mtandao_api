"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const dotenv_1 = __importDefault(require("dotenv"));
dotenv_1.default.config();
const express_1 = __importDefault(require("express"));
const http_1 = require("http");
const socket_io_1 = require("socket.io");
const socket_1 = require("./config/socket");
const db_1 = require("./config/db");
const stk_routes_1 = __importDefault(require("./routes/stk.routes"));
const auth_routes_1 = __importDefault(require("./routes/auth.routes"));
const category_routes_1 = __importDefault(require("./routes/category,routes"));
const message_route_1 = __importDefault(require("./routes/message.route"));
const business_routes_1 = __importDefault(require("./routes/business.routes"));
const product_routes_1 = __importDefault(require("./routes/product.routes"));
const sms_route_1 = __importDefault(require("./routes/sms.route"));
const auth_middleware_1 = require("./middleware/auth.middleware");
const body_parser_1 = __importDefault(require("body-parser"));
const cookie_parser_1 = __importDefault(require("cookie-parser"));
const user_model_1 = require("./models/user.model");
const cors_1 = __importDefault(require("cors"));
const path_1 = __importDefault(require("path"));
// dotenv.config();
const app = (0, express_1.default)();
app.use("/uploads", express_1.default.static(path_1.default.join(__dirname, "../public/uploads")));
app.use((0, cors_1.default)({ credentials: true, origin: ["http://localhost:3000", "https://marapesa.com", "https://spingofrontend.vercel.app", "https://api.marapesa.com"] }));
app.use((0, cookie_parser_1.default)());
app.use(body_parser_1.default.urlencoded({ extended: false }));
app.use(body_parser_1.default.json());
// const PORT = process.env.PORT || 5000;
const PORT = Number(process.env.PORT);
(0, db_1.connectDB)();
const httpServer = (0, http_1.createServer)(app);
const io = new socket_io_1.Server(httpServer, {
    cors: {
        origin: ["http://localhost:3000", "https://spingofrontend.vercel.app", "https://marapesa.com", "http://185.113.249.137:3000", "https://api.marapesa.com"],
        methods: ["GET", "POST"],
        credentials: true, // Allow credentials like cookies
        allowedHeaders: "Content-Type,Authorization", // Allow headers
    },
});
app.use("/api/auth", auth_routes_1.default);
app.use("/api/stk", stk_routes_1.default);
app.use("/api/business", auth_middleware_1.authenticateToken, business_routes_1.default);
app.use("/api/messages", auth_middleware_1.authenticateToken, message_route_1.default);
app.use("/api/sms", sms_route_1.default);
app.use("/api/products", auth_middleware_1.authenticateToken, product_routes_1.default);
app.use("/api/categories", auth_middleware_1.authenticateToken, category_routes_1.default);
app.get("/api/authenticated", auth_middleware_1.authenticateToken, async (req, res) => {
    let authuser = await user_model_1.User.findById(req.user.userId);
    res.json(authuser);
});
app.get("/api/protected", auth_middleware_1.authenticateToken, (req, res) => {
    res.json({ message: "This is a protected route", user: req.user });
});
app.get("/", (req, res) => {
    res.send("WebSocket Server is running!");
    return;
});
// app.use('/', swaggerUi.serve, swaggerUi.setup(swaggerSpec));
httpServer.listen(PORT, () => {
    console.log(`Swagger docs at http://localhost:${PORT}`);
});
console.log("PORT:", process.env.PORT);
(0, socket_1.setupSocket)(io);
